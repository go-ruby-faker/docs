...
`go-ruby-faker/faker` reimplements Ruby's `faker` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-faker/faker` directly, with no Ruby runtime;
